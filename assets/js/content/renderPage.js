/**
 * Renders page sections declared in bundled page config.
 */

import { renderContactForms } from "../contact/renderContactForms.js";
import { initContactFormSwitcher } from "../contact/initContactFormSwitcher.js";
...
function renderTestimonialRating(rating) {
  ...
  const displayValue = capped.toFixed(1);
  const stars = Array.from({ length: 5 }, (_, index) => {
    const icon = index < filled ? "bi-star-fill" : "bi-star";
    return `<i class="bi ${icon}" aria-hidden="true"></i>`;
  }).join("");

  return `
    <div class="testimonial-card-rating" aria-label="${escapeAttr(displayValue)} out of 5 stars">
      <span class="testimonial-card-rating-stars">${stars}</span>
      <span class="testimonial-card-rating-value">${escapeHtml(displayValue)}</span>
    </div>`;
}
...
function renderAboutSkillsSplitSection(data) {
  ...
  return `
    <section id="about-me" class="about-skills-split section">
      <div class="container">
...
function renderCareerSection(career) {
  ...
  return `
    <section id="experience" class="career-section section">
      <div class="container">
...
function renderAboutData(data) {
  ...
  const latestWorksSubtitle = escapeHtml(
    data.latestWorksSubtitle ?? data.subtitle ?? ""
  );
  ...
  const latestWorksSectionHtml = latestWorks.length
    ...
    <section id="latest-works" class="resume resume-layout section">
      <div class="container section-title" data-aos="fade-up">
        ...
        ${latestWorksSubtitle ? `<p>${latestWorksSubtitle}</p>` : ""}
      </div>
      <div class="container">
              ...
              <div class="swiper-slide">
                <article class="testimonial-card">
                  ...
                  <footer class="testimonial-card-footer">
                    ...
                    ${renderTestimonialRating(item.rating)}
                  </footer>
                </article>
              </div>
            `).join("")}
...
function renderContactData(data) {
  ...
  const formHtml = renderContactForms(data);
...
function initCareerTimeline(root) {
  ...
  function setExpandedBodyHeight(body, expanded) {
    if (!body) return;
    ...
    body.style.maxHeight = "0px";
    // Force reflow so the open animation starts from 0.
    void body.offsetHeight;
    body.style.maxHeight = `${fullHeight}px`;
  }

  function syncExpandedHeights() {
    document.querySelectorAll(".rl-career-exp-item.is-expanded .rl-career-exp-body").forEach((body) => {
      body.style.maxHeight = "none";
    ...
    items.forEach((item) => {
      ...
      toggle.addEventListener("click", () => {
        ...
        items.forEach((other) => {
          ...
          const otherBody = other.querySelector(".rl-career-exp-body");
          if (otherToggle) otherToggle.setAttribute("aria-expanded", "false");
          setExpandedBodyHeight(otherBody, false);
        });

        if (!isExpanded) {
          item.classList.add("is-expanded");
          toggle.setAttribute("aria-expanded", "true");
          setExpandedBodyHeight(body, true);
        }
      });
    });
  });

  if (!window.__careerTimelineResizeBound) {
    window.__careerTimelineResizeBound = true;
    window.addEventListener("resize", syncExpandedHeights);
  }

  // Layout can settle after fonts/AOS; sync once more shortly after bind.
  window.setTimeout(syncExpandedHeights, 300);
}
